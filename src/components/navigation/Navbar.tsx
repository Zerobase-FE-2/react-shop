...
export default function Navbar() {
  ...
  const NavBar = tw.nav`
  w-full h-16 p-2 bg-white dark:bg-gray-900 flex justify-between shadow-lg z-10 fixed
  `
  ...
  `
  const HomeBtn = tw.h1`
  inline text-black dark:text-white text-sm md:text-base lg:text-lg font-bold px-3 py-1
  `
  const NavBtn = tw.div`
  relative text-black dark:text-white invisible md:visible md:text-sm lg:text-base font-semibold px-3 py-2 mx-2 hover:bg-gray-200 dark:hover:bg-gray-700 hover:rounded-lg
  `
  ...
  <>
    <NavBar>
      ...
      <RightBar>
          <NavBtn onClick={() => setTheme(colorTheme)}>{colorTheme === 'light' ? <SunIcon className='w-7 h-7 mx-2' /> : <MoonIcon className='w-7 h-7 mx-2' />}</NavBtn>
          <SearchBar/>
          <Link to='/cart'>
            <NavBtn>
